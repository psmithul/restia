...
repair_app_tree_ownership() {
    if [ -d /app ]; then
        find /app -xdev \
            \( -path /app/data -o -path /app/logs -o -path /app/blobs -o -path /app/.ssh -o -path /app/.cache -o -path /app/.local -o -path /app/.npm \) -prune \
...
repair_app_tree_ownership
for dir in /app/data /app/logs /app/blobs /app/.ssh /app/.cache/huggingface /app/.local /app/.npm; do
