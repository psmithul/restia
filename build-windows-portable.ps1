...
<#
  Build a portable Windows distribution for Restia.

  Output layout:
    dist\Restia\Restia.exe
    dist\Restia\static\...
    dist\Restia\scripts\...
    dist\Restia\mcp_servers\...
    dist\Restia\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name Restia @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\Restia" -ForegroundColor Green
