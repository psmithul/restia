...
function _formatServeCmdPreview(cmd) {
  ...
  if (mlxDeepSeekV4Compat) {
    return [
      '# Restia runtime compatibility: using sanitized MLX DeepSeek-V4 shim.',
      formatted,
    ].join('\n');
  }
  return formatted;
...
function _rerenderCachedModels() {
  ...
  list.querySelectorAll('.memory-item[data-repo]').forEach(item => {
    item.addEventListener('click', (e) => {
      ...
      panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="prefix_cache"${sv('prefix_cache',false)?' checked':''} /> Prefix / Radix Cache${_h('vLLM: prefix caching. SGLang: RadixAttention prefix cache; when off Restia adds --disable-radix-cache.')}</label>`;
      ...
      // reflects the stored presets. Standard Restia .dropdown look, positioned
