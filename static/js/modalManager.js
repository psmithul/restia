...
const _LABELS = {
  ...
  'tasks-modal':       { label: 'Automations', icon: 'M9 11l3 3L22 4M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11' },
...
/** Minimize every visible registered tool before entering a full workspace. */
export function minimizeVisibleModals(excludedIds = []) {
  const excluded = new Set(Array.isArray(excludedIds) ? excludedIds : []);
  let minimized = 0;
  for (const id of Object.keys(_AUTO_WIRE)) {
    if (excluded.has(id)) continue;
    const modal = document.getElementById(id);
    if (!modal || modal.classList.contains('hidden') || modal.classList.contains('modal-minimized')) continue;
    const style = getComputedStyle(modal);
    if (style.display === 'none' || style.visibility === 'hidden') continue;
    if (!_state.has(id)) _autoRegister(id);
    if (minimize(id)) minimized += 1;
  }
  return minimized;
}
...
export default { register, unregister, isRegistered, isMinimized, minimize, restore, toggle, close, injectMinimizeButton, minimizeVisibleModals };
