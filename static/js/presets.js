...
async function _mergeUserMemories(charName) {
  try {
    ...
    const data = await res.json();
    const userMems = (data.memory || []).filter(m => (
      !m.character &&
      !m.readonly &&
      m.source !== 'restia_brain' &&
      !String(m.id || '').startsWith('brain:')
    ));
