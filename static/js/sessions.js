...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Restia Chat';
...
export async function selectSession(id, { keepSidebar = false, showLoading = true, immediateLoading = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Restia kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Restia Chat';
    }
