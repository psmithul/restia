#!/bin/bash
# Build a downloadable macOS launcher app + .dmg for Restia.
#
#   ./build-macos-app.sh
#
# Produces:
#   dist/Restia.app   — double-click: starts the local server (using this
#                         repo's venv) and opens the UI in an app-style window.
#   dist/Restia.dmg   — drag-to-Applications disk image (the downloadable).
#
...
# so rebuild if you move the repo. Override the port with RESTIA_PORT.
set -e
...
# ── Icon (best effort) — convert the current Restia app icon to .icns ──
if [ -f "$REPO_DIR/static/icons/icon-512.png" ] && command -v sips >/dev/null 2>&1; then
  ...
  if sips -s format icns "$TMPIMG/icon.png" --out "$APP/Contents/Resources/restia.icns" >/dev/null 2>&1; then
    echo "  icon:        restia.icns"
  else
    echo "  icon:        (skipped — conversion failed)"
  fi
  rm -rf "$TMPIMG"
else
  echo "  icon:        (skipped — no Restia app icon)"
fi
...
#!/bin/bash
# Restia.app — start the local server and open the UI in an app window.
...
LOG="$INSTALL_DIR/logs/restia-app.log"

notify() { /usr/bin/osascript -e "display notification \"$1\" with title \"Restia\"" >/dev/null 2>&1; }
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"Restia\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "Restia isn't set up yet. Open Terminal and run:
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "Restia failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "Restia is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Install:       open '$DIST/$APP_NAME.dmg'  (drag Restia to Applications)"
