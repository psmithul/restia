...
def _ensure_planner_calendar(db, owner: str) -> CalendarCal:
...
def _create_records(
    *,
    ...
    created: dict[str, Any] = {"note_id": None, "todo_note_id": None, "calendar_event_ids": []}
    db = SessionLocal()
    try:
        if update_notes:
            note = Note(
        ...
        if update_tasks:
            items = [{"id": str(uuid.uuid4())[:8], "text": item.get("text"), "done": False} for item in (plan.get("tasks") or []) if item.get("text")]
            todo = Note(
                id=str(uuid.uuid4()),
                owner=owner,
        ...
        if update_calendar:
            cal = _ensure_planner_calendar(db, owner)
            ...
            for item in plan.get("calendar") or []:
                ...
                ev = CalendarEvent(
                    uid=str(uuid.uuid4()),
                    calendar_id=cal.id,
                    summary=_clean_line(item.get("summary"), "Planner checkpoint"),
                    description=f"Planner checkpoint for: {goal}\n\nArtifact: {artifact_path}",
                    dtstart=start,
                    dtend=start + timedelta(minutes=duration),
                    all_day=False,
                    is_utc=False,
                    origin="local",
                    event_type="admin",
                ...
                db.add(ev)
                created["calendar_event_ids"].append(ev.uid)
        db.commit()
        return created
    except Exception:
        db.rollback()
...
def setup_planner_routes() -> APIRouter:
    ...
    async def run_planner(request: Request, body: PlannerRequest):
        ...
        artifact_path = plans_dir / f"{datetime.now().strftime('%Y%m%d-%H%M%S')}-{_safe_slug(goal)}-{str(uuid.uuid4())[:8]}.{fmt}"
        if fmt == "docx":
