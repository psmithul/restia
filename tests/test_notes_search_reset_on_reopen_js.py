...
def _open_panel_body():
    m_open = re.search(r"(?:export\s+)?function\s+openPanel\s*\([^)]*\)\s*{", SRC)
    assert m_open, "openPanel function not found"
    start = m_open.end()
    rest = SRC[start:]
...
def test_open_panel_resets_search_query():
    ...
    # reset must sit with the other new-open state resets.
    assert body.index("_editingId = null") < body.index("_searchQuery = ''") < body.index("const container")
