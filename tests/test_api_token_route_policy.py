...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "POST",
            "/api/life/automations/automation-1/evaluate",
            ["life:read"],
        ),
        ("GET", "/api/life/focus/current", ["life:write"]),
        ("GET", "/api/contacts/list", ["contacts:read"]),
        ("GET", "/api/contacts/search", ["contacts:write"]),
        ("POST", "/api/contacts/refresh", ["contacts:read"]),
        ("POST", "/api/contacts/add", ["contacts:write"]),
        (
            "POST",
            "/api/contacts/contact-1/resolve-conflict",
            ["contacts:write"],
        ),
        ("PUT", "/api/contacts/config", ["contacts:configure"]),
        ("PUT", "/api/contacts/contact-1", ["contacts:write"]),
        ("DELETE", "/api/contacts/contact-1", ["contacts:write"]),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "POST",
            "/api/life/automations/automation-1/prepare",
            ["life:read"],
        ),
        ("GET", "/api/contacts/list", ["chat"]),
        ("POST", "/api/contacts/refresh", ["chat"]),
        (
            "POST",
            "/api/contacts/contact-1/resolve-conflict",
            ["contacts:read"],
        ),
        ("POST", "/api/contacts/add", ["contacts:read"]),
        ("PUT", "/api/contacts/config", ["contacts:write"]),
    ],
)
