...
DATA_DIR = _env_alias("RESTIA_DATA_DIR", "ODYSSEUS_DATA_DIR", get_default_data_dir())

# Data file paths
# Single source of truth: every persisted file/dir lives under DATA_DIR, which
# is the ONLY place RESTIA_DATA_DIR / ODYSSEUS_DATA_DIR is read. Import these constants instead of
...
def internal_api_base() -> str:
    """Base URL for in-process loopback calls to Restia's own API.

    Agent tools and background jobs reach admin-gated routes by calling the
    running server over HTTP. Resolution order:
      1. RESTIA_INTERNAL_BASE / ODYSSEUS_INTERNAL_BASE - explicit override.
    ...
    """
    override = _env_alias("RESTIA_INTERNAL_BASE", "ODYSSEUS_INTERNAL_BASE")
    if override:
