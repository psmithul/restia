...
ASSISTANT_ALWAYS_AVAILABLE = frozenset({
    ...
    "manage_calendar", "manage_notes", "manage_tasks", "query_life",
...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "query_life": "Read-only canonical Life OS query surface across typed domains, the shared Today control plane, source-backed personal knowledge, graph links, and the unified Communications Hub. Use today with the user's exact UTC offset for what-to-do-next and day planning; it returns the same evidence, assumptions, and available actions as the Today screen. Use communications for owner-scoped unread/importance, summaries, draft suggestions, commitments, deadlines, contacts, and follow-ups; it never acknowledges or sends. Use proactive_report for explicit risk/digest inspection and knowledge_evidence for citation-backed claims that preserve stale, assumption, inference, contradiction, and gap labels. Generic list/search/get intentionally exclude typed knowledge so epistemic safeguards cannot be bypassed. It never mutates or executes; changes stay behind typed tools and reviewed Actions.",
    ...
    "send_email": "Prepare an exact outbound email behind mandatory Level-5 human review. It never sends or opens SMTP/IMAP; delivery can only be queued from Restia after fresh approval.",
    "reply_to_email": "Prepare an exact threaded reply behind mandatory Level-5 human review. Pass UID, recipient, subject, Message-ID, references, and account copied from read_email; it never sends, fetches IMAP, or marks the source Answered on the action path.",
    ...
    "manage_notes": "Create and manage the user's Notes and To-dos (two separate UI tabs). ALWAYS use this for note/todo/checklist/reminder creation — NEVER hit /api/notes via app_api. note_type picks the tab: 'todo' = things to DO (checkbox items in checklist_items), 'note' = things to KEEP (freeform content), 'goal' = tracked objective. Accepts natural-language `due_date` like 'tomorrow at 9am' or '11pm today' (parsed in the USER'S timezone). The due_date IS the reminder — it fires a notification at that time, so do NOT also create a calendar event for the same reminder. Set colors, labels, pin, archive. Do NOT use manage_memory for note content.",
    "manage_calendar": "Owner-scoped calendar event management: list, create, update, prepare a cancel_event proposal, and fail-closed manual-only delete. list_events returns uid + version; update_event and cancel_event MUST pass both exact values so stale edits fail safely. cancel_event never executes or exposes an approval token to the model; a human reviews it in Actions. Each event supports event_type, importance, all-day dates, and explicit recurrence via rrule. Calendar reminders are a separate manage_notes action and are never created as a hidden side effect.",
...
class ToolIndex:
    ...
    _KEYWORD_HINTS = {
        ...
        frozenset({"calendar", "event", "meeting", "schedule", "appointment"}):
            {"manage_calendar"},
        frozenset({"life os", "what should i do", "what matters", "goal", "goals",
                   "project", "projects", "milestone", "decision", "decisions",
                   "health trend", "habit", "habits", "routine", "routines",
                   "relationship", "relationships", "people", "follow up",
                   "journal", "reflection", "mood", "gratitude",
                   "home", "warranty", "insurance", "expiry", "renewal",
                   "travel", "trip", "itinerary", "packing", "lodging",
                   "learning", "career", "course", "skill gap", "portfolio",
                   "work workspace", "business workspace", "customer", "proposal",
                   "money", "finance", "task risk", "blocked task"}):
            {"query_life"},
