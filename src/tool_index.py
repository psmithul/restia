...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "query_life": "Read-only canonical Life OS query surface across typed domains, source-backed personal knowledge, graph links, and the unified Communications Hub. Use communications for owner-scoped unread/importance, summaries, draft suggestions, commitments, deadlines, contacts, and follow-ups; it never acknowledges or sends. Use proactive_report for deterministic what-next priorities and knowledge_evidence for citation-backed claims that preserve stale, assumption, inference, contradiction, and gap labels. Generic list/search/get intentionally exclude typed knowledge so epistemic safeguards cannot be bypassed. It never mutates or executes; changes stay behind typed tools and reviewed Actions.",
