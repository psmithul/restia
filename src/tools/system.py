...
async def do_app_api(content: str, owner: Optional[str] = None) -> Dict:
    """Generic loopback to allowed internal Restia API endpoints. Lets the
    ...
    if any(method == m and path.startswith(p) for m, p in _APP_API_BLOCKLIST_METHOD_PATH):
        ...
        if "/api/calendar/events" in path:
            return {"error": "Don't hit /api/calendar/events via app_api — use `manage_calendar` for owner-scoped events. For updates or cancellation proposals, pass the exact uid + version returned by list_events. cancel_event requires later human approval and delete_event stays manual-only. Calendar reminders are a separate explicit `manage_notes` action with due_date.", "exit_code": 1}
